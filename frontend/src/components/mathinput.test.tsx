...
describe('The MathInput', () => {
  it('displays the amount if provided', () => {
    ...
    expect(wrapper.find('#theInput_mathinput').props().value).toEqual('200');
  });
  it('displays the changed value', () => {
    const wrapper = shallow(
      <MathInput id="theInput" amount={100} onChange={e => undefined} />
    );
    wrapper.find('#theInput_mathinput').simulate('change', { target: { value: '12' } });
    expect(wrapper.find('#theInput_mathinput').props().value).toEqual('12');
  });
  it('displays the amount provided on focus loss', () => {
    ...
    wrapper.find('#theInput_mathinput').simulate('change', { target: { value: '12' } });
    wrapper.find('#theInput_mathinput').simulate('blur');
    expect(wrapper.find('#theInput_mathinput').props().value).toEqual('100');
  });
  ...
  it('calls the handler with number input', () => {
    ...
    wrapper.find('#theInput_mathinput').simulate('change', { target: { value: '1212' } });
  ...
  it('allows input to be modified by parent', () => {
    ...
    expect(wrapper.find('#theInput_mathinput').props().value).toBe(newAmount.toString());
  });
  ...
  it('doesn\'t allow parent to overwrite math in progress', () => {
    ...
    wrapper
      ...
      .simulate('change', { target: { value: '12*12' } });
    wrapper.setProps({ id: 'theInput', amount: newAmount, onChange: handler });
    expect(wrapper.find('#theInput_mathinput').props().value).toBe('12*12');
  });
});
